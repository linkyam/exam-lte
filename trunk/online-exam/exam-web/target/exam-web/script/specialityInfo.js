...
jQuery(document).ready(function() {
     ...
     * 初始学院信息下拉框
     */
    initCollegeSelect("collegeIdSearch",1);

    /**
     * 初始学制信息下拉框
     */
    initSpeYearSelect("speYearSearch",1);


    $('#myModal1').on('shown.bs.modal', function () {

        initCollegeSelect("collegeId",0);
        initSpeYearSelect("speyearsId",0);
    })
    ...
    $("#specialityInfoTable").on('click', '#delCollege', function () {
        ...
        $.messager.confirm("警告","此操作将删除"+data.collegeName+"所有信息！",function(){
...
function initCollegeSelect(id,bindSelect2) {
...
function initSpeYearSelect(id,bindSelect2){
...
function ajaxSubmit(frm) {
    ...
    com.ajax.jsonPost("/speciality/addSpecialityInfo",dataPara,function(data){
        if(data.code==com.ajax.jsonResult.success){
            $.messager.popup("新增专业信息成功！");
            cleanForm();
            initSpecialityInfoTableList();
        }else{
